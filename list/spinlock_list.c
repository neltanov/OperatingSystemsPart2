...
void add_node(Storage* storage, const char* new_value) {
    ...
    if (pthread_spin_init(&(new_node->lock), PTHREAD_PROCESS_PRIVATE) != 0) {
...
int lock_node(Node* node) {
    if (pthread_spin_lock(&node->lock)) {
        printf("lock_node: pthread_spin_lock failed\n");
        return 1;
    }
    return 0;
}
...
int unlock_node(Node* node) {
    if (pthread_spin_unlock(&node->lock)) {

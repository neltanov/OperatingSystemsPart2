...
void add_node(Storage* storage, const char* new_value) {
    ...
    if (pthread_rwlock_init(&(new_node->lock), PTHREAD_PROCESS_PRIVATE) != 0) {
        printf("add_node: pthread_spin_init failed\n");
        free(new_node);
        return;
    }
...
int lock_node(Node* node, int mode) {
    if (mode == READ) {
        if (pthread_rwlock_rdlock(&node->lock)) {
            printf("Some problems with lock_node in reader mode\n");
            return 1;
        }
    }
    else if (mode == WRITE) {
        if (pthread_rwlock_wrlock(&node->lock)) {
            printf("Some problems with lock_node in writer mode\n");
            return 1;
        }
    }
    else {
        printf("Invalid mode in argument\n");
        return 1;
    }
    return 0;
}
...
int unlock_node(Node* node) {
    if (pthread_rwlock_unlock(&node->lock)) {
        printf("unlock_node: pthread_spin_unlock failed\n");
        return 1;
    }
    return 0;
}
...
void *increment_asc_strings(void *arg) {
    ...
    while (1) {
        ...
        if (lock_node(storage->first, READ)) 
            return NULL;
        ...
        while (1) {
            ...
            if (lock_node(next, READ)) return NULL;
...
void *increment_desc_strings(void *arg) {
    ...
    while (1) {
        ...
        if (lock_node(storage->first, READ)) return NULL;
        Node* tmp = storage->first;
        while (1) {
            usleep(10);
            Node* next = tmp->next;
...
void *swap_nodes(void *arg) {
    ...
    while (1) {
        ...
        if (lock_node(storage->first, WRITE)) return NULL;
        else s1 = storage->first;

        if (lock_node(storage->first->next, WRITE)) return NULL;
        else s2 = storage->first->next;

        while (1) {
            Node* s3 = s2->next;
            if (lock_node(s3, WRITE)) return NULL;
